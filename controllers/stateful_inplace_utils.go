...
func isInPlaceOnly(set *nuwav1.StatefulSet) bool {
	return set.Spec.UpdateStrategy.RollingUpdate != nil &&
		set.Spec.UpdateStrategy.RollingUpdate.PodUpdatePolicy == nuwav1.InPlaceOnlyPodUpdateStrategyType
}
...
func shouldDoInPlaceUpdate(
	...
	if set.Spec.UpdateStrategy.RollingUpdate.PodUpdatePolicy != nuwav1.InPlaceIfPossiblePodUpdateStrategyType &&
		set.Spec.UpdateStrategy.RollingUpdate.PodUpdatePolicy != nuwav1.InPlaceOnlyPodUpdateStrategyType {
		return false, nil
	}
...
func checkInPlaceUpdateCompleted(pod *v1.Pod) error {
	inPlaceUpdateState := nuwav1.InPlaceUpdateState{}
	if stateStr, ok := pod.Annotations[nuwav1.StatefulSetInPlaceUpdateStateAnnotation]; !ok {
		return nil
...
func podInPlaceUpdate(pod *v1.Pod, spec *InPlaceUpdateSpec) (*v1.Pod, error) {
	...
	inPlaceUpdateState := nuwav1.InPlaceUpdateState{
		...
		LastContainerStatuses: make(map[string]nuwav1.InPlaceUpdateContainerStatus, len(spec.containerImages)),
	}
	for _, c := range newPod.Status.ContainerStatuses {
		if _, ok := spec.containerImages[c.Name]; ok {
			inPlaceUpdateState.LastContainerStatuses[c.Name] = nuwav1.InPlaceUpdateContainerStatus{
				ImageID: c.ImageID,
			}
		}
	}
	...
	newPod.Annotations[nuwav1.StatefulSetInPlaceUpdateStateAnnotation] = string(inPlaceUpdateStateJSON)
...
func shouldUpdateInPlaceReady(pod *v1.Pod) bool {
	...
	for _, r := range pod.Spec.ReadinessGates {
		if r.ConditionType == nuwav1.StatefulSetInPlaceUpdateReady {
...
func getInPlaceUpdateReadyCondition(pod *v1.Pod) *v1.PodCondition {
	for _, c := range pod.Status.Conditions {
		if c.Type == nuwav1.StatefulSetInPlaceUpdateReady {
			return &c
		}
	}
	return nil
