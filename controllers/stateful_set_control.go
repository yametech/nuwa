...
func (ssc *defaultStatefulSetControl) UpdateStatefulSet(set *nuwav1.StatefulSet, pods []*v1.Pod) error {
	...
	ssc.Log.Info("StatefulSe pod status",
		"ns", set.Namespace,
		"name", set.Name,
		"replicas", status.Replicas,
		"readyReplicas", status.ReadyReplicas,
		"currentReplicas", status.CurrentReplicas,
		"updateReplicas", status.UpdatedReplicas)

	ssc.Log.Info("StatefulSet revisions",
		"ns", set.Namespace,
		"name", set.Name,
...
func (ssc *defaultStatefulSetControl) ListRevisions(set *nuwav1.StatefulSet) ([]*apps.ControllerRevision, error) {
...
func (ssc *defaultStatefulSetControl) updateStatefulSet(
	...
	pods []*v1.Pod,
	revisions []*apps.ControllerRevision) (*nuwav1.StatefulSetStatus, error) {
	...
	for i := range replicas {
		...
		if !isCreated(replicas[i]) {
			if err := ssc.podControl.CreateStatefulPod(set, replicas[i]); err != nil {
				...
				condition := NewStatefulsetCondition(nuwav1.FailedCreatePod, v1.ConditionTrue, "", msg)
		...
		if shouldUpdateInPlaceReady(replicas[i]) {
			ssc.Log.Info(
				"StatefulSet update Pod InPlaceUpdateReady condition to true",
				"ns", set.Namespace,
				"name", set.Name,
				"podName", replicas[i].Name)
			newCondition := v1.PodCondition{
				Type:   nuwav1.StatefulSetInPlaceUpdateReady,
		...
		if err := ssc.podControl.UpdateStatefulPod(updateSet, replica); err != nil {
			...
			condition := NewStatefulsetCondition(nuwav1.FailedUpdatePod, v1.ConditionTrue, "", msg)
			SetStatefulsetCondition(&status, condition)
			return &status, err
		}
	}
...
func (ssc *defaultStatefulSetControl) inPlaceUpdatePod(set *nuwav1.StatefulSet, pod *v1.Pod, inPlaceUpdateSpec *InPlaceUpdateSpec) error {
	newCondition := v1.PodCondition{
		Type:   nuwav1.StatefulSetInPlaceUpdateReady,
